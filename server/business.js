...
export async function startBusiness() {
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  try {
   ...
   if(!['GET /api/checkin/status','POST /api/checkin','GET /api/membership','GET /api/products','POST /api/orders','GET /api/orders','POST /api/orders/cancel'].includes(req.method+' '+path) && !(req.method==='GET'&&path.startsWith('/api/products/')))return respond(res,404,{error:'接口尚未开放'});
   ...
   try {
    let data;
    if(path==='/api/checkin/status') data=await summary(client,user.id); else if(path==='/api/membership') data=await membership(client,user.id); else if(path==='/api/products') data={items:(await client.query('SELECT * FROM products WHERE active ORDER BY id')).rows}; else if(path==='/api/orders/cancel'){const b=await readBody(req); await client.query('BEGIN'); const o=(await client.query("UPDATE orders SET status='cancelled' WHERE id=$1 AND user_id=$2 AND status='pending' RETURNING *",[b.orderId,user.id])).rows[0]; if(!o) throw failure(409,'订单不可取消'); await client.query('UPDATE products SET stock=stock+1 WHERE id=$1',[o.product_id]); await client.query('COMMIT'); data=o;} else if(path==='/api/orders'&&req.method==='GET') data={items:(await client.query('SELECT o.*,p.name FROM orders o JOIN products p ON p.id=o.product_id WHERE o.user_id=$1 ORDER BY o.created_at DESC',[user.id])).rows}; else if(path==='/api/orders'){const b=await readBody(req); await client.query('BEGIN'); const p=(await client.query('SELECT * FROM products WHERE id=$1 AND active FOR UPDATE',[b.productId])).rows[0]; if(!p||p.stock<1) throw failure(409,'商品库存不足'); const o=(await client.query('INSERT INTO orders(user_id,product_id,amount) VALUES($1,$2,$3) RETURNING *',[user.id,p.id,p.price])).rows[0]; await client.query('UPDATE products SET stock=stock-1 WHERE id=$1',[p.id]); await client.query('COMMIT'); data=o;} else data=await checkin(client,user.id);
